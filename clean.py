...
def get_input_name(name):
	...
	while usr_input != 'y' and usr_input != 'Y':
		...
		usr_input = input('Name will be changed to: \''
						  + full_name + '\'. Enter Y to continue: ')
	return full_name
...
def get_input_approval(name, target_name):
	...
	'''
	usr_input = input('Enter Y to approve name change from \''
					  + name + '\' to \'' + target_name + '\': ')
...
def clean_coach_names():
	...
	with user input, and writes the data to a file specified by the user
	'''
	args = parsing()
	with open(args.rawfile) as f:
		soup = BeautifulSoup(f, 'html.parser')
	coach_tags = soup.find_all('name', text = re.compile('coach'))
	coach_names = sorted([coach.text.split()[1:] for coach 
						  in set(coach_tags)], key = lambda x: x[-1])
	...
	for name in coach_names:
		...
		shared_names = set([tag.text for tag in soup.find_all(
			'name', text = re.compile(last_name))]).difference([target_name])
		for name in shared_names:
			usr_approved = get_input_approval(name, target_name)
			regex_name = '^' + name + '$'
			if usr_approved: 
				length += len(soup.find_all(
					'name', text = re.compile(regex_name)))
				for name_tag in soup.find_all(
					'name', text=re.compile(regex_name)):
					name_tag.string.replace_with(target_name)
		assert len(soup.find_all(
			'name', text = re.compile(target_name))) == length
